...
class MedianAlignedTierAgent(FinanceSimulationAgent):
    ...
    def _apply_roundtrip_completion_prices(
        ...
        *,
        ...
        """
        if self.roundtrip_complete_ticks <= 0 or not book.bids or not book.asks:
            return bid, ask
        ...
        improve = self.roundtrip_complete_ticks * tick
        if fingerprint.signed_position > 0:
    ...
    def _execute_plan(
        ...
        for order_id in plan.cancel_ids:
            ...
            response.cancel_order(book_id, order_id)
            acted = True
        bid_qty = max(plan.bid_qty, floor) if plan.place_bid else 0.0
        if plan.place_bid and account.quote_balance.free >= bid_qty * plan.bid_price:
            if self._book_instruction_count(response, book_id) >= cap:
                if acted:
                    fingerprint.last_plan_ts = state.timestamp
                return
            ...
            fingerprint.last_bid_price = plan.bid_price
            acted = True
        ask_qty = max(plan.ask_qty, floor) if plan.place_ask else 0.0
        if plan.place_ask and account.base_balance.free >= ask_qty:
            ...
            fingerprint.last_ask_price = plan.ask_price
            acted = True
        if acted:
            fingerprint.last_plan_ts = state.timestamp
